...
interface ByMarketOvtChartProps {
    snDataChart: any;
    cableDataChart: any;
    big4DataChart: any;
  }
...
export function ByMarketOvtChart({ snDataChart, cableDataChart, big4DataChart }: ByMarketOvtChartProps) {
    const [dynamicYAxis, setDynamicYAxis] = useState<boolean>(true);
    const [marketFilter, setMarketFilter] = useState<string>('Total');
  
    const uniqueMarkets = big4DataChart.map((item:any) => item['Market / Region']);
    ...
    useEffect(() => {
      if (snDataChart.length > 0 && cableDataChart.length > 0 && big4DataChart.length > 0) {
        ...
        const snChartData = snDataChart.find((item:any) => item['Market / Region'] === marketFilter);
        const cableChartData = cableDataChart.find((item:any) => item['Market / Region'] === marketFilter);
        const big4ChartData = big4DataChart.find((item:any) => item['Market / Region'] === marketFilter);
  
        if (!snChartData || !cableChartData || !big4ChartData) {
          console.error('Data not found for selected market');
          return;
        }
        ...
        const SnChartDataFinal = labels.map(label => snChartData[label] as number);
        const CableChartDataFinal = labels.map(label => cableChartData[label] as number);
        const Big4ChartDataFinal = labels.map(label => big4ChartData[label] as number);
      ...
      <div> 
        <div className="flex justify-around items-center mb-5 mr-6">
          {uniqueMarkets && uniqueMarkets.length > 0 && (
            ...
            >
              {uniqueMarkets.map((market:any, index:any) => 
